...
\subsubsection{Struttura di un Neurone}

Per un singolo neurone l'output è calcolato come:

\begin{equation}
    \text{output} = f\!\left(\mathbf{W}\mathbf{x} + \mathbf{b}\right)
\end{equation}
...
\end{equation}
Ideale per gli hidden layer: computazionalmente efficiente e non soffre del
problema del gradiente che svanisce per valori positivi. Ha però il problema
dei \textit{neuroni morti} --- se l'input è sempre negativo il neurone smette
di attivarsi e il suo gradiente è sempre zero.
...
i valori del vettore contemporaneamente. Non ha senso applicarla a un singolo
neurone isolato in un hidden layer, perché l'operazione $e^{x_i}/\sum_j e^{x_j}$
su un singolo numero darebbe sempre 1. Negli hidden layer si usano funzioni
...
= 54.000$ training). Caso classico di \textbf{rendimenti decrescenti}.
...
\begin{description}
    ...
    \item[Vanishing gradient] Problema per cui i gradienti diventano
        estremamente piccoli nelle prime fasi del training, rallentando
        o impedendo l'apprendimento (tipico di Sigmoid su reti profonde).
\end{description}
